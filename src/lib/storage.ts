...
export function getRecentRecipes(): ParsedRecipe[] {
  try {
    ...
    return recipes.sort((a, b) => new Date(b.parsedAt).getTime() - new Date(a.parsedAt).getTime());
  } catch (error) {
 ...
 */
export function addRecentRecipe(recipe: Omit<ParsedRecipe, 'id' | 'parsedAt'>): void {
  try {
