...
export async function POST(req: NextRequest): Promise<Response> {
  try {
    ...
    if (!allowedTypes.includes(imageFile.type)) {
      console.error(
        '[API /parseRecipeFromImage] Invalid file type:',
        imageFile.type,
      );
      return NextResponse.json(
        formatError(
    ...
    if (imageFile.size > maxSize) {
      console.error(
        '[API /parseRecipeFromImage] File too large:',
        imageFile.size,
      );
      return NextResponse.json(
        formatError(
    ...
    console.log(
      '[API /parseRecipeFromImage] Converted image to base64, length:',
    ...
    if (!result.success || !result.data) {
      console.error(
        '[API /parseRecipeFromImage] Parsing failed:',
        result.error,
      );
