...
export function cleanRecipeHTML(rawHtml: string): CleanedHTML {
  try {
    ...
    // Find recipe title
    const recipeTitle =
      $('h1, .recipe-title, [class*="recipe-title"], [itemprop="name"]').first().text().trim() ||
    ...
    $(".breadcrumb, .breadcrumbs, .breadcrumb-container, .breadcrumbs-container").remove();
    ...
    $(".wp-block-group, .wp-block-buttons, .wp-block-embed, .widget").each((_, element) => {
      const $element = $(element);
      if (
    ...
    $(".search, .search-box, .search-container, .search-form, .site-search, .search-bar").remove();

    // Remove app banners and mobile prompts
    $(".app-banner, .mobile-banner, .mobile-sticky, .open-app, .app-link, .download-app").remove();
    ...
    $(".tooltip, .tooltips, .hint, .hovercard, .dropdown-menu, .dropdown").remove();
    ...
    if (!remainingContent || remainingContent.trim().length === 0) {
      remainingContent =
        $('div[class*="content"], div[class*="post"], div[class*="entry"]').first().html() || "";
    }
...
export function extractTextContent(rawHtml: string): string {
  try {
    ...
    logger.error({ err: error }, "Error extracting text content");
    return "";
  }
}
